...
void handle_warnings(void)
{
	if((adc_values . vo_sense > pmbus_dcdc_config_translated[0]. vout_ov_warn_limit))
	{
		pmbus_status_word |= (0x8000) | (0x1);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
	if((adc_values . vo_sense < pmbus_dcdc_config_translated[0]. vout_uv_warn_limit))
	{
		pmbus_status_word |= (0x8000) | (0x1);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
	if((adc_values_avg . io_sense > pmbus_dcdc_config_translated[0]. iout_oc_warn_limit))
	{
		pmbus_status_word |= (0x4000) | (0x10) | (0x1);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
	if((adc_values_avg . ips > pmbus_dcdc_config_translated[0]. iin_oc_warn_limit))
	{
		pmbus_status_word |= (0x2000) | (0x1);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
	if((adc_values_avg . pri_temp < pmbus_dcdc_config_translated[0]. temp_ot_warn_limit))
	{
		pmbus_status_word |= (0x4);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
	if((adc_values_avg . vin_mon > pmbus_dcdc_config_translated[0]. vin_ov_warn_limit))
	{
		pmbus_status_word |= (0x2000) | (0x1);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
	if((adc_values_avg . vin_mon < pmbus_dcdc_config_translated[0]. vin_uv_warn_limit))
	{
		pmbus_status_word |= (0x2000) | (0x1);
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert ( Pull the pin down)
	}
...
void handle_pgood(void)
{
	if ((adc_values_avg . vo_sense > pmbus_dcdc_config_translated[0]. pgood_on_limit))
	{
		MiscAnalogRegs.GLBIOVAL.all |= ((1 << (8)));
		pmbus_status_word &= ~((0x1) | (0x800));
	}
	else if ((adc_values_avg . vo_sense < pmbus_dcdc_config_translated[0]. pgood_off_limit))
	{
		MiscAnalogRegs.GLBIOVAL.all &= ~((1 << (8)));
		pmbus_status_word |= (0x1) | (0x800);
	}
...
//	Uint8 shut_down_fault_detected = 0;
	...
	if(((adc_values . pri_temp > pmbus_dcdc_config_translated[0]. temp_ot_fault_limit)) || ((adc_values . sec_temp > (pmbus_dcdc_config_translated[0]. temp_ot_fault_limit + 30))))
	...
	if((adc_values . vin_mon > pmbus_dcdc_config_translated[0]. vin_ov_fault_limit))
	{
		pmbus_status_word |= (0x2000) | (0x1);				
//		shut_down_fault_detected = 1;
	}
	if(((adc_values . vin_mon < pmbus_dcdc_config_translated[0]. vin_uv_fault_limit) && (supply_state == STATE_REGULATED)))
	{
		pmbus_status_word |= (0x2000) | (0x1);				
		shut_down_fault_detected = 1;
	}
	if(((adc_values . vo_sense < pmbus_dcdc_config_translated[0]. vout_uv_fault_limit) && (supply_state == STATE_REGULATED)))
	{
		pmbus_status_word |= (0x8000) | (0x1);	
		PMBusRegs.PMBCTRL3.bit.ALERT_EN = 1;  //Set SMBus alert (Pull the pin down)	
		shut_down_fault_detected = 1;
