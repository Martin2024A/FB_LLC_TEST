...
Uint8 start_erase_task(const void* dest_ptr, Uint16 byte_count)
{
	...
	erase_segment_counter = (byte_count + (32)-1) / (32);
...
{                                                                                           
	...
	if(   (dest_addr > (0x18fff)) 
		|| (dest_addr < (0x18800)) )
	{

		return (1);	// Error: Starting addr out of range
	}

	// Verify that the ending address is within the valid memory range
	// 16-bit byte_count can't force the end to "wrap".
	if(dest_addr+byte_count-1 > (0x18fff))
	{
		return (1);	// Error: Ending addr out of range
	}

	// Calculate which segment(s) of Data Flash to erase
	if(first_segment != (0))
	{
		*first_segment = (Uint8)( (dest_addr - (0x18800)) / (32) );
	}
