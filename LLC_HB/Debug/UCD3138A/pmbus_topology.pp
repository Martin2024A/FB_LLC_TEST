...
const Uint8 status_bit_mask[]    = {0x00, 0x00, 0x00, 0x80, 0x00, 0x80, 0x80, 0x34, 0xC8, 0x00, 0x00};
...
{
	if(pmbus_read)
		return pmbus_read_one_byte_handler((32 - (9)));
	else
		...
		case STATE_CPCC:
			{
				...
				MiscAnalogRegs.GLBIOVAL.all &= ~((Uint32)((1 << (28))));
				return 0;
			}
		case STATE_IDLE:
			{
				pmbus_status_word = (0x1) |
									(0x40) |
									(0x800);
				fiq_data.fault_status = 0;
				PMBusRegs.PMBCTRL3.bit.ALERT_EN = 0;
...
inline Uint8 pmbus_read_vout(void)
{
    ...
    struct qnote vout_rev_adc_scaler = {(28180), (-13)};


	vout_adc12_value = qnote_linear16_multiply_fit(vout_rev_adc_scaler, adc_values . vo_sense, 0, (0x0FFFF));
...
inline Uint8 pmbus_read_vin(void)
{
	...
	struct qnote vin_rev_adc_scaler = {(21045), (-20)};  //1/49.7
