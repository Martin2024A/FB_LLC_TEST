...
void pmbus_handler(void)
{
	...
	else if(pmbus_state == 1)
	{
		pmbus_write_block_handler();
		return;
	}
	else if(pmbus_state == 2)
	{
		pmbus_read_block_handler();
		return;
	}
	else if(pmbus_state == 3)
	{
...
void pmbus_write_block_handler(void)
{
	...
	if((pmbus_status_half_word_0_value & (((0x20) + (0x08) + (0x10) + (0x80) + (0x0100) + (0x0200)) + (0x07))) == 0) 
	{
		return; //if nothing has happened, just return;
	}
	else if((pmbus_status_half_word_0_value & (((0x20) + (0x08) + (0x10) + (0x80) + (0x0100) + (0x0200)) )) ==
			((0x20) + (0x08) + (0x80)))
			//end of message, good data ready, pec valid, must be 2 to 4 bytes
	{
		...
		pmbus_number_of_bytes = pmbus_number_of_bytes +
								(pmbus_status_half_word_0_value & (0x07));
		//pmbus_write_message();
		pmbus_read_write_message((0));
		PMBusRegs.PMBACK.byte.BYTE0 = 1;  //ack these 4 bytes;
		pmbus_state = 0;
		return;
	}
	else if((pmbus_status_half_word_0_value & (((0x20) + (0x08) + (0x10) + (0x80) + (0x0100) + (0x0200)) + (0x07))) ==
			...
			//no end of message, good data ready, 4 bytes of data - must be a send block 
	{
		...
		if(pmbus_number_of_bytes >= 50) //if it's too big 
...
void pmbus_idle_handler()
{
	...
	if((pmbus_status_half_word_0_value & (((0x20) + (0x08) + (0x10) + (0x80) + (0x0100) + (0x0200)) + (0x07))) == 
				(0x20))
			...
			//end of message, good data ready, pec valid, must be 2 to 4 bytes
	{
		...
		pmbus_number_of_bytes = pmbus_status_half_word_0_value & (0x07);
		//pmbus_write_message();
		pmbus_read_write_message((0));
...
//			PMBusRegs.PMBACK.byte.BYTE0 = 1;  //ack command;
			...
			PMBusRegs.PMBCTRL2.byte.BYTE2 = (0x08) + pmbus_number_of_bytes
											+ (0x60);
...
//			PMBusRegs.PMBACK.byte.BYTE0 = 1;  //ack command;
			...
			PMBusRegs.PMBCTRL2.byte.BYTE2 =  4 + (0x60); //4 is number of bytes
...
void pmbus_read_block_handler(void)
{
...

	{
		...
		{
			PMBusRegs.PMBCTRL2.byte.BYTE2 = 1 + (0x08)
											+ (0x60);  //send byte and PEC;
		...
		if(pmbus_buffer_position >= pmbus_number_of_bytes) //if last byte
		{
			PMBusRegs.PMBCTRL2.byte.BYTE2 = 2 + (0x08)
											+ (0x60);  //send bytes and PEC;
			PMBusRegs.PMBTXBUF.all = u.pmbus_transmit_load; //
			pmbus_state = 3;
			return  ;
		}
		...
		if(pmbus_buffer_position >= pmbus_number_of_bytes) //if last byte
		{
			PMBusRegs.PMBCTRL2.byte.BYTE2 = 3 + (0x08)
											+ (0x60);  //send bytes and PEC;
			PMBusRegs.PMBTXBUF.all = u.pmbus_transmit_load; //
			pmbus_state = 3;
			return  ;
		...
		if(pmbus_buffer_position >= pmbus_number_of_bytes) //if last byte
		{
			PMBusRegs.PMBCTRL2.byte.BYTE2 = 4 + (0x08)
											+ (0x60);  //send bytes and PEC;
			PMBusRegs.PMBTXBUF.all = u.pmbus_transmit_load; //
			pmbus_state = 3;
			return  ;
		}
		PMBusRegs.PMBCTRL2.byte.BYTE2 = 4 + (0x60) ;  //send bytes, no PEC;
		PMBusRegs.PMBTXBUF.all = u.pmbus_transmit_load; //
...
void pmbus_read_wait_for_eom_handler(void)
{
	...
	else if((pmbus_status_half_word_0_value & (((0x20) + (0x08) + (0x10) + (0x80) + (0x0100) + (0x0200)) + (0x07))) == (0x20))
	{//if eom, done
		pmbus_state = 0;
		PMBusRegs.PMBACK.byte.BYTE0 = 1;  //ack them
		return ;
